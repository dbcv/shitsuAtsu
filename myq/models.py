...
class Photo(models.Model):
    ...
    uploaded_at = models.DateTimeField(
        verbose_name="アップロード日時", auto_now_add=True
    )
    ...
    def get_image_url_256(self):
        return reverse(
            "serve_photo", kwargs={"uuid": self.uuid, "width": 256, "ext": "webp"}
        )
...
class SegmentedPhoto2(models.Model):
    ...
    image = models.ImageField(
        verbose_name="切り抜き画像",
        upload_to=upload_to_segmented_path,
        blank=True,
        null=True,
    )
...
class SegmentedPhoto(models.Model):
    ...
    def get_image_url(self):
        return reverse(
            "serve_segmented_photo", kwargs={"uuid": self.uuid, "ext": "webp"}
        )

    def get_image_url_256(self):
        return reverse(
            "serve_segmented_photo",
...
class AccessLog(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="ユーザー"
    )
